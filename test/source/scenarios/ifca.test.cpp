...
void await_results_concurently(std::vector<InputType> &input,
  ...
  for (auto &&in : input) {
    auto future = std::async(
        std::launch::async,
        [&results, &results_mutex](InputType &&in) {
          in.wait();
...
TEST_CASE("IFCA- Filtering tests") {
  ...
  SUBCASE("Dropping chunks in the middle of chain") {
    ...
    std::mutex m;
    std::function<bool(in)> checkCall = [&m, &secondFuncCalled](in chunk) {
      LOG_DEBUG() << "wchodzi dla: " << chunk;
